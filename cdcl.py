...
class cdclprev():
    def __init__(self, sentence, num_vars, vsids, lrb, chb):
        ...
        self.eliminate_subsumed_clauses()
    ...
    def signature(self, clause):
        cl_bits = 0
        for l in clause:
            l = int(abs(l) % 64)  # hash function
            l_bits = 1 << l
            cl_bits |= l_bits
    
        return cl_bits

    def subsumes(self, clause1, clause2):
        if (self.cl_sig[clause1] & ~self.cl_sig[clause2]) != 0:
            return False
        else:
            for lit in self.sentence[clause1]:
                if self.sentence[clause2].count(lit) == 0:
                    return False
        return True

    def find_subsumed(self, subset):
        subsumed = []
        min_size = sys.maxsize
        min_lit = 0
        for lit in self.sentence[subset]:
            if len(self.l2c_watch[lit]) < min_size:
                min_size = len(self.l2c_watch[lit])
                min_lit = lit
        # consider the literal that appears least often

        supers = self.l2c_watch[min_lit]
        for super in supers:
            if (self.subsumes(subset, super)):
                subsumed.append(super)
        return subsumed

    def eliminate_subsumed_clauses(self):
        for clause1 in range(0, len(self.sentence)):
            for clause2 in self.find_subsumed(clause1):
                if (clause1 != clause2):
                    for lit in self.sentence[clause2]:
                        if clause2 in self.l2c_watch[lit]: self.l2c_watch[lit].remove(clause2)
                    self.sentence[clause2] = []
                    self.c2l_watch[clause2] = []  

    def self_subsume(self):
        for var in range(-self.num_vars, self.num_vars + 1):
            for clause1 in self.l2c_watch[var]:
                subset = self.sentence[clause1].copy()
                if subset == []: continue
                subset.remove(var)
                subset.append(-var)
                self.sentence.append(subset)
                tmp=len(self.sentence)-1
                self.cl_sig[len(self.sentence)-1] = self.signature(subset)
                for clause2 in self.find_subsumed(len(self.sentence)-1):
                    new_cl = self.sentence[clause2].copy()
                    new_cl.remove(-var)
                    self.sentence.append(new_cl)
                    for lit in self.sentence[clause2]:
                        if clause2 in self.l2c_watch[lit]: self.l2c_watch[lit].remove(clause2)
                    self.sentence[clause2] = []
                    self.c2l_watch[clause2] = []
                self.sentence.remove(subset)
                del self.cl_sig[tmp]  
    ...
    def init_watch(self):
        ...
        for clause_idx, clause in enumerate(self.sentence):  # for each clause watch first two literals
            ...
            self.cl_sig[clause_idx]=self.signature(clause)
            for lit in clause:
