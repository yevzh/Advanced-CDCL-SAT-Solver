...
class Bandit:
    ...
    def UCB(self):
        ...
        while 1:
            ...
            if t < self.num_arms*3:
                ...
                arm = t%3
                if arm == 0:
                    solver = cdcl(self.sentence, self.num_vars, self.vsids_scores, self.lrb_scores, self.chb_scores)
            ...
            for i in range(self.num_arms):
                ucb[i] = self.emp_means[i]+ np.sqrt(4*np.log(t)/self.num_pulls[i])
            arm = np.argmax(ucb)
            print("best")
            print(arm)
            if arm == 0:
    ...
    def EXP3(self):
        ...
        prob = np.ones(self.num_arms)
        gamma = 1
        while 1:
            print(t)
            print("limit",self.conflict_limit)
            for i in range(self.num_arms):
                prob[i] = (1-gamma)*weight[i]/np.sum(weight) + gamma/self.num_arms
                
            arm = 0
            rand = random.uniform(0,1)
            if rand<prob[0]:
                arm = 0
            if rand>=prob[0] and rand< prob[0]+prob[1]:
                arm = 1
...
class Banditt:
    ...
    def UCB(self):
        ...
        arm = np.argmax(ucb)
        if arm == 0:
            solver = cdclprev(self.sentence, self.num_vars, self.vsids_scores, self.lrb_scores, self.chb_scores)
